...
t "decide appends under ## Decisions"  bash -c "crn decide 431 'selftest decision' && awk '/^## Decisions/,/^## Context/' $T/b/work/platform-431.md | grep -q 'selftest decision'"
...
t "Context section opens with the node" bash -c "crn open 431 | grep -q '^## Context' && crn open 431 | grep -q 'egress breakdown.*→ ../../files/'"
t "lazy files stay outside the bundle" bash -c "! crn find 'peering plan' | grep -q 'files/\|artifacts/'"
t "runbook line lives on the system"   bash -c "crn open systems/database | grep -q '^- compact a collection.*→'"
