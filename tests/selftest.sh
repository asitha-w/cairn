...
t "graph json has edges"              bash -c "crn graph --format json >/dev/null && python3 -c 'import json;g=json.load(open(\"$T/b/.cairn/graph.json\"));assert g[\"counts\"][\"edges\"]>3 and any(e[\"rel\"]==\"blocked_by\" for e in g[\"edges\"])'"
